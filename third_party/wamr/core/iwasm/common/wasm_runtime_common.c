...
wasm_runtime_env_init()
{
    ...
    gettimeofday(&end_time, NULL);
    // printf("runtime signal init cost %ld us\n",(end_time.tv_sec - start_time.tv_sec) * 1000000 + (end_time.tv_usec - start_time.tv_usec));
#endif
...
wasm_runtime_full_init(RuntimeInitArgs *init_args)
{
    ...
    gettimeofday(&end_time, NULL);
    // printf("runtime env init cost %ld us\n",(end_time.tv_sec - start_time.tv_sec) * 1000000 + (end_time.tv_usec - start_time.tv_usec));
    ...
    gettimeofday(&end_time, NULL);
    // printf("runtime register natives cost %ld us\n",(end_time.tv_sec - start_time.tv_sec) * 1000000 + (end_time.tv_usec - start_time.tv_usec));
...
void*
wasm_runtime_create_memory(uint32_t total_size) {
    uint64 map_size = 8 * (uint64)BH_GB;
    void *res;

    if (!(res = os_mmap(NULL, map_size, MMAP_PROT_NONE, MMAP_MAP_NONE))) {
       printf("map error\n");
    }

    if (os_mprotect(res, total_size, MMAP_PROT_READ | MMAP_PROT_WRITE) != 0) {
        os_munmap(res, map_size);
        printf("set map prot failed\n");
    }
